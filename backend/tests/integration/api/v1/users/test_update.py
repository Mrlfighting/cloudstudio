...
from .test_register import generate_register_data
...
async def test_update_user_profile_wrong_user(
    ...
    other_reg_data = generate_register_data("other")
    create_response = await auth_client.post("/api/v1/users/register", json=other_reg_data)
    assert create_response.status_code == 201
    other_username = other_reg_data["account"]
...
async def test_update_user_profile_duplicate_email(
    ...
    response = await superuser_auth_client.patch(f"/api/v1/users/{username}", json=update_data)
