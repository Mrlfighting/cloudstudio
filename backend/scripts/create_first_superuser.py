...
async def create_first_superuser() -> None:
    ...
    try:
        ...
        async with local_session() as session:
            ...
            user_data = UserCreate(name=name, email=email, username=username, password=password, user_profile=None)
            ...
            # user_role 为角色唯一来源，is_superuser 同步为 admin
            stmt = update(User).where(User.id == user_id).values(is_superuser=True, user_role="admin")
