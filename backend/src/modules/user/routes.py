...
@router.post(
    ...
    summary="创建用户（仅管理员）",
    description="""
           仅管理员可用：创建新用户（可指定昵称、邮箱、密码等）。

           公开注册请使用 POST /api/v1/users/register（账号 + 密码 + 确认密码）。
           新用户默认角色为 user，管理员可通过 PATCH /users/{username}/role 调整。
           """,
    responses={
        201: {"description": "用户创建成功"},
        403: {"description": "无权限 - 需要管理员"},
        422: {"description": "参数校验失败或账号/邮箱已存在"},
    },
...
@router.post(
    ...
    summary="用户注册",
    description="""
           公开注册：账号 + 密码 + 确认密码（可选昵称、简介）。

           注册成功后需调用 POST /auth/login 登录获取会话 cookie。
           """,
    responses={
        201: {"description": "注册成功"},
        422: {"description": "参数校验失败（密码不一致/格式错误/账号已存在）"},
    },
...
@router.patch(
    ...
    summary="修改用户角色（仅管理员）",
    description="""
           仅管理员可用：修改用户角色（user / admin）。

           user_role 为角色唯一来源，is_superuser 自动同步。
           """,
    responses={
        200: {"description": "角色修改成功"},
        403: {"description": "无权限 - 需要管理员"},
        404: {"description": "用户不存在"},
        422: {"description": "非法角色值"},
    },
    response_description="操作结果提示",
)
async def update_user_role(
    ...
    """管理员修改用户角色。"""
    try:
        await user_service.update_role(username, values, db)
        return {"message": "用户角色修改成功"}
    except Exception as e:
        http_exception = handle_exception(e)
        if http_exception:
            raise http_exception
