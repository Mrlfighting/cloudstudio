...
class UserService:
    ...
    async def register(self, data: UserRegister, db: AsyncSession) -> dict[str, Any]:
        """用户公开注册：账号 + 密码 + 确认密码。

        Args:
            data: 注册数据（账号、密码、确认密码，可选昵称与简介）。
            db: 数据库会话。

        Returns:
            创建成功的用户数据字典（UserRead）。

        Raises:
            UserExistsError: 账号已被占用。
        """
        account_exists = await crud_users.exists(db=db, username=data.account)
        if account_exists:
            raise UserExistsError(f"账号 '{data.account}' 已被占用")

        user_internal = UserCreateInternal(
            username=data.account,
            name=data.nickname or data.account,
            email=None,
    ...
    async def update_role(self, username: str, role_update: UserRoleUpdate, db: AsyncSession) -> dict[str, Any]:
        """管理员修改用户角色（user_role 为准，is_superuser 显式同步）。

        Args:
            username: 目标用户账号。
            role_update: 新角色（"user" / "admin"）。
            db: 数据库会话。
        ...
        # 角色同步：user_role 为角色唯一来源，is_superuser 显式保持一致（FastCRUD 走 Core UPDATE，不依赖模型事件）
        updated_user = await crud_users.update(
            db=db,
            object={
                "user_role": role_update.user_role,
                "is_superuser": role_update.user_role == "admin",
            },
            id=user["id"],
